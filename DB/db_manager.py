...
SHORT_SCHEMA = LANGUAGES + SKILLS + PSYCH_FACTOR + GRADES + EXPERIENCE + STUDY_GROUP + PROJECTS
SCHEMA = SHORT_SCHEMA + ROLES + CREDENTIALS
...
class DbManager:
    ...
    def insert_student(self, record):
        self.db.prepare(
            """insert into records ({}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}) 
                                            ...
                                            (select role_id from project_roles where role = $16), $17, $18)"""
                .format(*record.keys()))(*record.values())
    ...
    def get_student(self, student_id):
        ...
        return Record.Record({x[1]: x[0] for x in zip(row[1:len(row)-2], SCHEMA)})

    def get_students(self):
        records = list(self.db.query("select * from records"))
        return list([Record.Record({x[1]: x[0] for x in zip(row[1:], SCHEMA)}) for row in records])
    ...
    def get_student_record(self, student_id=None, email=None, password=None):
        ...
        return Record.Record({x[1]: x[0] for x in zip(row[1:], SCHEMA)})
    ...
    def get_max_ids(self):
        sg = self.db.query("select max(group_id) from study_group")[0]
        pj = self.db.query("select max(project_id) from project")[0]
        lg = self.db.query("select max(language_id) from languages")[0]
        pr = self.db.query("select max(role_id) from project_roles")[0]
        ex = self.db.query("select max(experience_id) from experience")[0]
        pf = self.db.query("select max(factor_id) from psych_factor")[0]
